...
async function main() {
  ...
  // ─── Inject homepage hero images into index.html ───────────────────────────
  // The new magazine landing has a placeholder CITY_HERO_IMAGES object in
  // index.html. We replace it here so the 4 featured cities get real photos.
  var FEATURED = ['lisbon', 'copenhagen', 'barcelona', 'rome'];
  var INDEX_PATH = path.join(__dirname, 'index.html');
  if (fs.existsSync(INDEX_PATH)) {
    var heroMap = {};
    FEATURED.forEach(function(key){
      var safe = normKey(key);
      if (results[safe] && results[safe].url) heroMap[key] = results[safe].url;
    });
    var heroJs = 'var CITY_HERO_IMAGES = ' + JSON.stringify(heroMap, null, 4) + ';';
    var indexHtml = fs.readFileSync(INDEX_PATH, 'utf-8');
    // Replace the placeholder CITY_HERO_IMAGES = { ... } object.
    // The placeholder is wrapped between two markers so this is unambiguous.
    var startMarker = 'var CITY_HERO_IMAGES = {';
    var endMarker = '};';
    var start = indexHtml.indexOf(startMarker);
    if (start >= 0) {
      var end = indexHtml.indexOf(endMarker, start);
      if (end >= 0) {
        var before = indexHtml.substring(0, start);
        var after = indexHtml.substring(end + endMarker.length);
        var patched = before + heroJs + after;
        fs.writeFileSync(INDEX_PATH, patched);
        console.log('Patched index.html with '+Object.keys(heroMap).length+' homepage hero images: '+Object.keys(heroMap).join(', '));
      } else {
        console.warn('Could not find end of CITY_HERO_IMAGES in index.html — skipping homepage patch');
      }
    } else {
      console.warn('CITY_HERO_IMAGES not found in index.html — skipping homepage patch');
    }
  }
}
